...
int binary_search(const int * const array, const int lenght, const int step, 
    const int search);
int binary_search_recursive(const int * const array, int left, int right, 
    const int length, const int search);
...
main(void) 
{
  ...
  while (!WindowShouldClose()) 
  {
    ...
    BeginDrawing();
    {
      ...
      binary_search(array, length, step, 2);
    }
    EndDrawing();
  }
  CloseWindow();
...
int
binary_search(const int *const array, const int length, const int step,
    const int search) 
{
  int low = 0;
  int high = length - 1;
  ...
  while (low <= high) 
  {
    ...
    int gaps = middle * GAP_SIZE;
    int span = length * GAP_SIZE;

    DrawRectangle(gaps + HALF_WIDTH - span / 2,
        START_HEIGHT - TALL * array[middle], FAT,
        array[middle] * TALL, (Color){0xF0, 0xF0, 0x6F, 0xFF});

    if (array[middle] == search) 
    {
      DrawRectangle(gaps + HALF_WIDTH - span / 2,
          START_HEIGHT - TALL * array[middle], FAT,
          array[middle] * TALL, GREEN);
      return middle;
    } 
    else if (search < array[middle]) high = middle;
    else low = middle + 1;
  }
  return -1;
}
...
int
binary_search_recursive(const int *const array, int left, int right,
    const int length, const int search) 
{
  if (left <= right) 
  {
    ...
    if (search < array[middle])
      return binary_search_recursive(array, left, middle, length, search);
    else
      return binary_search_recursive(array, middle + 1, right, length, search);
  }
  return -1;
}
